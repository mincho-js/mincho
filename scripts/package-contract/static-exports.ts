...
function hasPresetProperty(expression: ts.Expression): boolean {
  if (ts.isParenthesizedExpression(expression)) {
    return hasPresetProperty(expression.expression);
  }
  return (
    ...
    if (
      ...
      for (const element of statement.exportClause.elements) {
        if (
          ...
          namespaceImports.has((element.propertyName ?? element.name).text)
        ) {
          unsupported(element.name.text);
        }
      }
    }
    if (
      ts.isExportAssignment(statement) &&
      hasPresetProperty(statement.expression)
    ) {
      unsupported("preset");
    }
    if (
      ...
      /preset/i.test(statement.exportClause.name.text)
    ) {
      unsupported(statement.exportClause.name.text);
    }
    if (
      ts.isExportDeclaration(statement) &&
      statement.moduleSpecifier !== undefined
