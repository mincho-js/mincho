...
type HasPossibleThemeContractShape<ThemeTokens extends Theme> =
  Extract<ThemeTokens["vars"], object> extends never
    ? false
    : Extract<ThemeTokens["values"], object> extends never
      ? false
      : Extract<ThemeTokens["cssVarByPath"], object> extends never
        ? false
        : true;

// Reserve possible contract shapes in direct calls; arrays are token values.
type ThemeCreationTokensInput<ThemeTokens extends Theme> =
  ThemeTokensInput<ThemeTokens> &
    (HasPossibleThemeContractShape<ThemeTokens> extends true
      ? Extract<
          ThemeTokens[keyof ThemeContract],
          readonly unknown[]
        > extends never
        ? never
        : unknown
...
if (import.meta.vitest) {
  ...
  describe.concurrent("theme", () => {
    ...
    it("reserves contract fields with possible object values", () => {
      const tokens = {
        vars: { brand: "blue" },
        values: { brand: "red" },
        cssVarByPath: { brand: "green" }
      ...
      const unionVars = {
        ...tokens,
        vars: tokens.vars as string | typeof tokens.vars
      };
      const unionValues = {
        ...tokens,
        values: tokens.values as number | typeof tokens.values
      };
      const unionCSSVarByPath = {
        ...tokens,
        cssVarByPath: tokens.cssVarByPath as
          | undefined
          | typeof tokens.cssVarByPath
      };
      const assertReservedInputTypes = () => {
        // @ts-expect-error: vars can contain a contract object.
        theme(unionVars);
        // @ts-expect-error: values can contain a contract object.
        theme(unionValues);
        // @ts-expect-error: cssVarByPath can contain a contract object.
        theme(unionCSSVarByPath);
        // @ts-expect-error: typed handles reserve possible contract objects.
        theme.with<typeof unionVars>()(unionVars);
        // @ts-expect-error: typed handles reserve possible contract objects.
        theme.with<typeof unionValues>()(unionValues);
        // @ts-expect-error: typed handles reserve possible contract objects.
        theme.with<typeof unionCSSVarByPath>()(unionCSSVarByPath);
      };

      assertType<() => void>(assertReservedInputTypes);
      for (const input of [unionVars, unionValues, unionCSSVarByPath]) {
        expect(() => callThemeForRuntimeError(input)).toThrow(
          THEME_CONTRACT_DIRECT_CALL_ERROR
        );
      }
    });
