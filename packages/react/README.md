...
Dynamic declaration leaves support direct runtime values, conditional leaves, and compiler-recognized one-hole suffix templates for CSS Values & Units Level 4 units, plus CSS Containment Level 3 container query units: `%`, `cap`, `ch`, `cm`, `cqb`, `cqh`, `cqi`, `cqmax`, `cqmin`, `cqw`, `deg`, `dpcm`, `dpi`, `dppx`, `dvb`, `dvh`, `dvi`, `dvmax`, `dvmin`, `dvw`, `em`, `ex`, `fr`, `grad`, `Hz`, `ic`, `in`, `kHz`, `lh`, `lvb`, `lvh`, `lvi`, `lvmax`, `lvmin`, `lvw`, `mm`, `ms`, `pc`, `pt`, `px`, `Q`, `rad`, `rcap`, `rch`, `rem`, `rex`, `ric`, `rlh`, `s`, `svb`, `svh`, `svi`, `svmax`, `svmin`, `svw`, `turn`, `vb`, `vh`, `vi`, `vmax`, `vmin`, `vw`, and `x`. Examples include `` `${props.gap}px` ``, `` `${props.width}%` ``, `` `${props.size}rem` ``, `` `${props.rotate}deg` ``, and `` `${props.inlineSize}cqw` ``. The transform emits generated `style` entries that call `vx(value, suffix?)`; `vx` is generated-code support from `@mincho-js/transform-runtime`, not a user-facing CSS API. Unit suffix matching is ASCII case-insensitive, matching CSS unit parsing.
