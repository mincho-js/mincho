...
Explicit `css` props with pre-css or post-css spreads are supported in both direct and nested JSX positions. Direct return and expression-statement contexts keep statement-hoist lowering with no IIFE. Nested contexts such as expression-bodied arrows, conditionals, logical/nullish operands, call arguments, fragment or element children, JSX attribute expression containers, and unbraced control-flow consequents use a local zero-argument arrow IIFE to preserve JavaScript evaluation order while still compiling CSS away before React handles JSX.

The guardrails are intentional: spread-only `css` such as `<div {...{ css: styleA }} />` is not transformed or stripped because there is no explicit `css` prop on the element; explicit `key`/`ref` on spread css-prop elements are rejected; fragment targets and unsupported JSX targets still fail; nested `await` or `yield` inside expressions moved into the local IIFE are rejected; and Mincho does not provide Emotion-style runtime serializer, cache, style insertion, helper library behavior, or full runtime spread parity.
