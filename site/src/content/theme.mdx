...
const [themeClass, vars, contract] = theme({
  color: {
    primary: "#007bff",
    secondary: "#6c757d",
  },
...
The `theme()` function returns a tuple containing a className, resolved token references, and a structural `ThemeContract` used for contract-first replacement themes.
...
// contract: ThemeContract object for theme.extends(contract, nextTokens)
```

Apply the theme by adding the className to a container element:

```tsx
<div className={themeClass}>{/* Themed content */}</div>
```

Create a replacement theme from the returned contract when you need a second class that reuses the same CSS custom properties:
...
const [className, vars, contract] = theme(tokens);
const [nextClassName, nextVars, nextContract] = theme.extends(
  contract,
  nextTokens,
);
```

`theme()` and callable `theme.with<T>()` handles reserve the top-level shape
`{ vars: {...}, values: {...}, cssVarByPath: {...} }` for structural theme contracts
when all three fields are plain objects. This combination is rejected as direct
token input by TypeScript and at runtime. Use `theme.extends()` for contracts;
for token groups with those names, nest them under another key or rename a group.
The individual names remain available, and using a primitive or array for any
of the three fields does not form the reserved shape.
...
```ts
const [lightTheme, lightVars, lightContract] = theme(
  {
    color: {
      background: "#ffffff",
      text: "#000000",
...
const [darkTheme, darkVars, darkContract] = theme(
  {
    color: {
      background: "#1a1a1a",
      text: "#ffffff",
...
// Use lightContract or darkContract for contract-first replacement themes
```
...
`theme.with<T>()` creates a typed theme factory and returns the same `[className, vars, contract]` tuple as `theme()`. Its callable handle also extends a contract without repeating `T`:

```ts
type MyTheme = { color: { primary: string } };

const handle = theme.with<MyTheme>();
const tokens = {
  color: { primary: "#007bff" },
};
const [themeClass, vars, contract] = handle(tokens);

const replacementTokens = {
  color: { primary: "#4dabff" },
};
const optionalDebugId = "replacementTheme";
const [replacementThemeClass, replacementVars, replacementContract] =
  handle.extends(contract, replacementTokens, optionalDebugId);
```

Additional helpers such as `theme.raw()` and `theme.multiple()` are planned for future releases. See [Literal CSS](/docs/literal#css-methods) and [Rules](/docs/rules#multiple-rules) for examples of how helper methods work with other APIs.
