...
// contract: ThemeContract object for theme(contract, nextTokens)
```
...
const [nextClassName, nextVars, nextContract] = theme(contract, nextTokens)
```
...
`theme.with<T>()` creates a typed theme factory and returns the same `[className, vars, contract]` tuple as `theme()`:

```ts
const createTheme = theme.with<{ color: { primary: string } }>()
const [themeClass, vars, contract] = createTheme({
  color: { primary: '#007bff' }
})
```
